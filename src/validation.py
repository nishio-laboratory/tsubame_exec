...
def validate_exec_config(config) -> None:
    ...
    if "max_runtime" not in config["exec"]["resource"]:
        print(
            colored(
                f"exec.max_runtime not defined. defaulting to 00:04:59"
                , "red")
        )
        config["exec"]["max_runtime"] = "23:59:59"
    if "type" not in config["exec"]["resource"]:
        print(colored(f"exec.resource.type not defined. defaulting to cpu_4", "red"))
    ...
    if "group" not in config["exec"]["env"]:
        print(colored("exec.env.group not specified. defaulting to tga-nlab", "red"))
...
def validate_minimal(config: dict) -> None:
    if "exec" not in config:
        raise Exception("exec table not defined. you must at minimum specify exec.cmd and exec.env.dir")
    if "env" not in config["exec"]:
        raise Exception("exec.env table not defined. you must specify exec.env.dir")
